...
if [ .$load_schema = ."y" ]; then
	...
	sudo -u postgres psql -h $database_host -p $database_port -U freeswitch -d freeswitch -f /www/wwwroot/fusionpbx/resources/install/sql/switch.sql -L /tmp/schema.log;
fi

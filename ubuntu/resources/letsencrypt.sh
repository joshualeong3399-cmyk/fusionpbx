...
mkdir -p /www/wwwroot/dehydrated
...
#ensure BaoTA nginx ssl dir
mkdir -p /www/server/nginx/conf/ssl

#update BaoTA nginx config
sed "s@ssl_certificate         /etc/ssl/certs/nginx.crt;@ssl_certificate /etc/dehydrated/certs/$domain_alias/fullchain.pem;@g" -i /www/server/nginx/conf/sites-available/fusionpbx
sed "s@ssl_certificate_key     /etc/ssl/private/nginx.key;@ssl_certificate_key /etc/dehydrated/certs/$domain_alias/privkey.pem;@g" -i /www/server/nginx/conf/sites-available/fusionpbx

#reload nginx
systemctl reload nginx || /www/server/nginx/sbin/nginx -s reload
...
if [ .$switch_tls = ."true" ]; then
	...
	#set the permissions
	chown -R www:www /etc/freeswitch/tls

...
#change the version of php for arm (optional, BaoTA should handle this)
...
if [ -f "$nginx_sites_available/fusionpbx" ]; then
	...
	case "$pv" in
		7.1|71)
			sed -i -e 's#unix:.*;#unix:/www/server/php/71/var/run/php-fpm.sock;#g' "$nginx_sites_available/fusionpbx"
			;;
		7.3|73)
			sed -i -e 's#unix:.*;#unix:/www/server/php/73/var/run/php-fpm.sock;#g' "$nginx_sites_available/fusionpbx"
			;;
		7.4|74)

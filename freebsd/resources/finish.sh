...
/www/server/pgsql/bin/psql --host=$database_host --port=$database_port --username=$database_username -c "insert into v_domains (domain_uuid, domain_name, domain_enabled) values('$domain_uuid', '$domain_name', 'true');"
	...
	if [ .$nginx_enabled = .'true' ]; then
		cd /usr/local/www/fusionpbx && /usr/bin/php /www/wwwroot/fusionpbx/core/upgrade/upgrade.php --defaults
	fi
...
/www/server/pgsql/bin/psql --host=$database_host --port=$database_port --username=$database_username -t -c "insert into v_vars (var_uuid, var_name, var_value, var_category, var_order, var_enabled) values ('4507f7a9-2cbb-40a6-8799-f8f168082585', 'local_ip_v4', '$local_ip_v4', 'Defaults', '0', 'true');";
